...
import com.ibm.util.CustomErrorType;
	...
	// -------------------Retrieve All Employees---------------------------------------------
	    ...
	    public ResponseEntity<?> createEmployee(@RequestBody Employee employee, UriComponentsBuilder ucBuilder) {
	        ...
	        if (employeeService.isEmployeeExist(employee)) {
	            logger.error("Unable to create employee with name {} already exist", employee.getFirstName());
	            return new ResponseEntity(new CustomErrorType("Unable to create employee with firstname " + 
	            employee.getFirstName() + " already exist."),HttpStatus.CONFLICT);
	        }
